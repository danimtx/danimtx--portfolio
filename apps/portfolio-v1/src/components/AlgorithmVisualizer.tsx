...
export const AlgorithmVisualizer: React.FC = () => {
  ...
  const resetVisualizer = () => {
    setIsAutoRunning(false);
    setActiveStep(0);
    setVisitedNodes(algoSteps[0].visited);
    setActiveEdges([algoSteps[0].activeEdge]);
  };
    ...
    <div className="algo-visualizer-card">
      <div className="algo-header">
        ...
        <p className="algo-desc">
          {lang === 'en'
            ? 'Top 20 in Bolivia National Finals 2025 (Team RISE - UPDS) and South America Regional Finalist 2024 (Team Dijkstraidos - UAJMS). Deep mastery of advanced graph theory, dynamic programming, and asymptotic bounds (O((V + E) log V), O(N log N)).'
            : 'Top 20 en la Final Nacional Bolivia 2025 (Equipo RISE - UPDS) y Finalista Sudamérica 2024 (Equipo Dijkstraidos - UAJMS). Dominio en estructuras de datos avanzadas, teoría de grafos y optimización asintótica (O((V + E) log V)).'}
        </p>
      </div>
      ...
      <div className="algo-body-grid">
        ...
        <div className="algo-graph-box">
          <div className="algo-graph-header">
            ...
            <div className="algo-controls-group" style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
              <button
                type="button"
                className="algo-step-btn"
                onClick={togglePlayPause}
                aria-label={isAutoRunning ? (lang === 'en' ? 'Pause simulation' : 'Pausar simulación') : (lang === 'en' ? 'Resume simulation' : 'Reanudar simulación')}
              >
                {isAutoRunning ? (lang === 'en' ? '⏸ Pause' : '⏸ Pausa') : (lang === 'en' ? '▶ Auto' : '▶ Auto')}
              </button>
              <button
                type="button"
                className="algo-step-btn"
              ...
              <button
                ...
                aria-label={lang === 'en' ? 'Reset algorithm' : 'Reiniciar algoritmo'}
              >
                ↺
              </button>
            </div>
