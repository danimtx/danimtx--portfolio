      ...
      <div style={{ display: activeProject ? 'none' : 'block' }}>
        ...
        <div className="html-content">
            ...
            <div className="hero-cyber-grid gsap-reveal">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
                ...
                <div className="hero-actions">
                  ...
                  <a href={activeWhatsappLink} target="_blank" rel="noreferrer" className="btn-primary" style={{ borderColor: 'var(--accent-cyan)', color: 'var(--accent-cyan)' }}>
                    <FiSend size={18} />
                    {t.heroBtnContact}
                  </a>
          ...
          <section className="tech-grid-container section" id="skills" data-sector="about">
            ...
            <div className="rail-left" {...setHover('about')}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
                ...
                <div className="tech-level" data-about-level="1">
                  ...
                  <div className="gsap-reveal" style={{ maxWidth: '850px', margin: '0 0 2.5rem 0', textAlign: 'left' }}>
                    <p style={{ fontSize: '1.25rem', color: 'var(--text-color)', lineHeight: 1.85 }}>
                      {t.coreBioP1}
                    </p>
                    <p style={{ fontSize: '1.1rem', color: 'var(--text-secondary)', lineHeight: 1.75, marginTop: '1rem' }}>
                      {t.coreBioP2}
                    </p>
                  </div>
                  ...
                  <div className="tech-group">
                    <h3 className="tech-group-title">
                      <FaServer size={18} />
                      {skillCategories[0].name} — <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{skillCategories[0].tag}</span>
                    </h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>{skillCategories[0].description}</p>
                ...
                <div className="tech-level" data-about-level="2">
                  <div className="tech-group">
                    <h3 className="tech-group-title">
                      <FaReact size={18} />
                      {skillCategories[1].name} — <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{skillCategories[1].tag}</span>
                    </h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>{skillCategories[1].description}</p>
                    <div className="tech-grid">
                ...
                <div className="tech-level" data-about-level="3">
                  <div className="tech-group">
                    <h3 className="tech-group-title">
                      <FaRobot size={18} />
                      {skillCategories[2].name} — <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{skillCategories[2].tag}</span>
                    </h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>{skillCategories[2].description}</p>
                    <div className="tech-grid">
          ...
          <section className="section" id="projects" data-sector="projects" style={{ marginTop: '5vh' }}>
            ...
            <div className="rail-left" {...setHover('projects')}>
              <SystemsShowcase 
          ...
          <section className="section" id="certificates" data-sector="achievements" style={{ marginTop: '5rem' }}>
            <h2 className="huge-title gsap-reveal" style={{ top: '-5%', left: 'auto', right: '5%' }}>{t.achievementsHeading}</h2>

            <div className="rail-left" {...setHover('achievements')}>
              ...
              <h3 className="gsap-reveal" style={{ marginBottom: '2rem', fontSize: '2rem', color: 'var(--text-color)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap: '2rem', width: '100%', marginBottom: '4rem' }}>
                {mainCertificates.map((cert) => (
                  <div key={cert.id} className="certificate-card gsap-reveal">
                    ...
                    <h4>{cert.title}</h4>
                    {cert.institution && (
                      <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '0.78rem', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '0.4rem' }}>
              ...
              <h3 className="gsap-reveal" style={{ marginBottom: '2rem', fontSize: '1.8rem', color: 'var(--accent-cyan)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(250px, 100%), 1fr))', gap: '2rem', width: '100%' }}>
                {otherCertificates.map((cert) => (
                  <div key={cert.id} className="certificate-card gsap-reveal">
                    ...
                    {cert.institution && (
                      <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '0.3rem' }}>
                        {cert.institution}
                      </div>
                    )}
                  </div>
          ...
          <section className="section" id="contact" data-sector="contact" style={{ minHeight: '90vh', alignItems: 'center' }}>
            <div className="gsap-reveal contact-panel" {...setHover('contact')}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontFamily: '"JetBrains Mono", monospace', fontSize: '0.8rem', color: 'var(--accent-cyan)', marginBottom: '1.5rem' }}>
                <span className="hero-telemetry-dot" />
                <span>{t.contactStatus}</span>
              </div>

              <p style={{ fontSize: '1.15rem', color: 'var(--text-secondary)', lineHeight: 1.75, marginBottom: '2.5rem' }}>
                {t.contactDesc}
              </p>

              <div className="contact-row">
                <a href={`mailto:${personalInfo.email}`} className="view-btn contact-link" style={{ fontSize: '1.05rem' }}>
                  <span style={{ color: 'var(--accent-cyan)' }}>{t.contactEmailLabel}</span> {personalInfo.email}
                </a>
                <a href={activeWhatsappLink} target="_blank" rel="noreferrer" className="view-btn" style={{ fontSize: '1.05rem', color: 'var(--accent-cyan)' }}>
                  <span style={{ color: 'var(--accent-cyan)' }}>{t.contactWhatsappLabel}</span> {personalInfo.whatsapp}
                </a>
              </div>
